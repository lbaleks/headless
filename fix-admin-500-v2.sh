#!/usr/bin/env bash
# fix-admin-500-v2.sh — LiteBrygg Admin (Next.js 15)
# Unblocks 500s, scaffolder manglende admin-ruter, og lager /api/pricing/rules + /api/health.
# Denne versjonen unngår mange heredocs ved å la ett Node-skript skrive alle filer.
...
# 3) Skriv alle filer via ett Node-skript (EN HEREDOC) — ikke rør indent på EOF!
node - <<'__NODE__'
...
const mkPage = (title:string, feature:string) => `import dynamic from 'next/dynamic'
...
if (!(await fs.stat('.').then(_=>true).catch(_=>false))) {}
if (!await fs.stat('next.config.ts').then(_=>true).catch(_=>false)
 && !await fs.stat('next.config.mjs').then(_=>true).catch(_=>false)
 && !await fs.stat('next.config.js').then(_=>true).catch(_=>false)) {
  await w('next.config.ts', nextconfig)
}
...
const pricingPage = path.join(root, 'app/admin/pricing/page.tsx')
try { await fs.stat(pricingPage) } catch {
...
const health = await fetch(base+'/api/health').then(ok).then(j); assert.equal(health.ok,true)
const rules  = await fetch(base+'/api/pricing/rules').then(ok).then(j); assert.ok(Array.isArray(rules.rules))
...
const updated = await fetch(base+'/api/pricing/rules',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({id,name:'Auto Test',type:'margin',value:0.25,enabled:true})}).then(ok).then(j); assert.equal(updated.value,0.25)
...
# 4) Tips for VS Code/mac PTY:
#  - Ikke indentér EOF-markøren (__NODE__) eller legg mellomrom etter den.
#  - Pass på at “smart quotes” er AV i VS Code når du limer inn shell.
...
3) Hurtig-API-sjekk (ny terminal):
