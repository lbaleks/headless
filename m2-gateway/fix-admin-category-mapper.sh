...
: "${ADMIN_DIR:?Fant ikke admin-mappa (admstage).}"

echo "➡️  Admin: $ADMIN_DIR"

# Sørg for at sti finnes
mkdir -p "$ADMIN_DIR/app/m2/categories"

# Sørg for .env.local og base-url
touch "$ADMIN_DIR/.env.local"
grep -q '^NEXT_PUBLIC_GATEWAY_BASE=' "$ADMIN_DIR/.env.local" || \
  echo "NEXT_PUBLIC_GATEWAY_BASE=http://localhost:3044" >> "$ADMIN_DIR/.env.local"

# Skriv/overskriv siden
...
echo "➡️  Åpne:  http://localhost:3000/m2/categories"
