...
# Overwrite admin.ui.js with safe client-side id refs
cat > "$UI" <<'JS'
...
'  const r=await api("/v2/integrations/magento/products?page=1&pageSize=12"); const j=await r.json(); const items=j.items||[];'+
'  var cards="";'+
...
'    "<div class=\\"mb-4 flex items-center justify-between\\"><h1 class=\\"text-xl font-semibold\\">Products</h1></div>"+'+
...
'  const r=await api("/v2/integrations/magento/orders?page=1&pageSize=12"); const j=await r.json(); const list=j.items||[];'+
'  var rows="";'+
...
'  });'+
'  document.getElementById("view").innerHTML = '+
'    "<div class=\\"mb-4 flex items-center justify-between\\"><h1 class=\\"text-xl font-semibold\\">Orders</h1></div>"+'+
'    "<div class=\\"card overflow-auto\\"><table class=\\"min-w-full text-sm\\">"+'+
'      "<thead><tr class=\\"text-left text-slate-500\\"><th class=\\"py-2 px-2\\">ID</th><th class=\\"py-2 px-2\\">Increment</th><th class=\\"py-2 px-2\\">Status</th><th class=\\"py-2 px-2\\">Total</th><th></th></tr></thead>"+'+
...
'          "<thead><tr class=\\"text-left text-slate-500\\"><th class=\\"py-1 px-2\\">Item ID</th><th class=\\"py-1 px-2\\">SKU</th><th class=\\"py-1 px-2\\">Name</th><th class=\\"py-1 px-2\\">Inv.</th><th class=\\"py-1 px-2\\">Ref.</th><th class=\\"py-1 px-2\\">Price</th></tr></thead>"+'+
'          "<tbody>"+rows+"</tbody></table>"+'+
'      "</div>"+'+
'    "</div>"+'+
'    "<script>document.getElementById(\\"btn-invoice\\").onclick=async function(){var r=await fetch(\\"/v2/integrations/magento/orders/"+'+ 'id' +'+"/invoice\\",{method:\\"POST\\",headers:{\\"content-type\\":\\"application/json\\",\\"x-role\\":\\"admin\\",\\"Idempotency-Key\\":\\"ui-inv-"+Date.now()+"\\"},body:\\"{\\\\\\"capture\\\\\\":true}\\"}); var j=await r.json(); showNotice(\\"Invoice: \\"+JSON.stringify(j));};<\/script>"+'+
'    "<script>document.getElementById(\\"btn-refund\\").onclick=async function(){var r=await fetch(\\"/v2/integrations/magento/orders/"+'+ 'id' +'+"/creditmemo/full\\",{method:\\"POST\\",headers:{\\"content-type\\":\\"application/json\\",\\"x-role\\":\\"admin\\",\\"Idempotency-Key\\":\\"ui-crm-"+Date.now()+"\\"},body:\\"{\\\\\\"notify\\\\\\":false,\\\\\\"appendComment\\\\\\":false,\\\\\\"refund_shipping\\\\\\":false}\\"}); var j=await r.json(); showNotice(\\"Refund: \\"+JSON.stringify(j), j.ok===true);};<\/script>";'+
'};'+

'async function viewInvoices(){'+
'  const r=await api("/v2/integrations/magento/invoices?page=1&pageSize=10"); const j=await r.json(); const list=j.items||[];'+
'  var rows="";'+
'  list.forEach(function(x){rows+= "<tr class=\\"border-b\\"><td class=\\"py-1 px-2\\">"+x.entity_id+"</td><td class=\\"py-1 px-2\\">"+x.order_id+"</td><td class=\\"py-1 px-2\\">"+fmtMoney(x.grand_total)+"</td><td class=\\"py-1 px-2 text-right\\"><a class=\\"btn btn-soft\\" href=\\"#/invoice/"+x.entity_id+"\\">Open</a></td></tr>";});'+
'  document.getElementById("view").innerHTML = "<h1 class=\\"text-xl font-semibold mb-3\\">Invoices</h1><div class=\\"card overflow-auto\\"><table class=\\"min-w-full text-sm\\"><thead><tr class=\\"text-left text-slate-500\\"><th class=\\"py-1 px-2\\">ID</th><th class=\\"py-1 px-2\\">Order</th><th class=\\"py-1 px-2\\">Total</th><th></th></tr></thead><tbody>"+rows+"</tbody></table></div>";'+
'};'+

'async function viewInvoice(id){'+
'  const r=await api("/v2/integrations/magento/invoices/"+id); const j=await r.json(); const x=j.invoice||{};'+
'  document.getElementById("view").innerHTML = "<div class=\\"flex items-center justify-between mb-4\\"><h1 class=\\"text-xl font-semibold\\">Invoice #"+id+"</h1><a class=\\"btn btn-soft\\" href=\\"#/invoices\\">Back</a></div><div class=\\"card\\"><div>Order: "+(x.order_id||"")+"</div><div>Total: <b>"+fmtMoney(x.grand_total)+"</b></div></div>";'+
'};'+
...
'  document.getElementById("view").innerHTML = "<h1 class=\\"text-xl font-semibold mb-3\\">Credit Memos</h1><div class=\\"card\\"><div>ID: "+(x.entity_id||"4")+"</div><div>Order: "+(x.order_id||"")+"</div><div>Total: <b>"+fmtMoney(x.grand_total||0)+"</b></div></div>";'+
'};'+

'async function viewMSI(){'+
'  const sku="TEST";'+
'  const a=await api("/v2/integrations/magento/msi/source-items/"+sku).then(function(r){return r.json()});'+
'  const b=await api("/v2/integrations/magento/msi/salable-qty/"+sku+"?stockId=1").then(function(r){return r.json()});'+
'  var rows=""; (a.items||[]).forEach(function(it){rows+= "<tr class=\\"border-b\\"><td class=\\"py-1 px-2\\">"+it.source_code+"</td><td class=\\"py-1 px-2\\">"+(it.status? "In stock":"Out")+"</td><td class=\\"py-1 px-2\\">"+it.quantity+"</td></tr>";});'+
