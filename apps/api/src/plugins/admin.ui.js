...
<header class="sticky top-0 z-20 bg-slate-900 text-white">
  <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
    ...
    <nav class="flex gap-2">
      <a href="#/orders" class="tag">Orders</Link>
      <a href="#/products" class="tag">Products</Link>
      <a href="#/flags" class="tag">Feature Flags</Link>
      <Link href="/v2/docs">Docs</Link>
    <a href="#/roles" class="tag">Roles</Link><a href="#/activity" class="tag">Activity</Link></nav>
  </div>
</header>
...
const api=(p,opt)=>{ 
  ...
  async function viewProduct(sku){
    ...
    document.getElementById('view').innerHTML =
      '<div class="flex items-center justify-between mb-4"><h1 class="text-xl font-semibold">'+(p.name||sku)+'</h1><a class="btn btn-soft" href="#/products">Back</Link></div>'+
  ...
  async function viewOrders(){
    ...
    var rows=''; list.forEach(function(o){rows+='<tr><td>'+o.entity_id+'</td><td>'+(o.increment_id||'')+'</td><td>'+(o.status||'')+'</td><td>'+fmt(o.grand_total)+'</td><td class="text-right"><a class="btn btn-soft" href="#/order/'+o.entity_id+'">Open</Link></td></tr>'});
    document.getElementById('view').innerHTML =
  ...
  async function viewOrder(id){
    ...
    document.getElementById('view').innerHTML =
      '<div class="flex items-center justify-between mb-4"><h1 class="text-xl font-semibold">Order #'+(o.increment_id||id)+'</h1><a class="btn btn-soft" href="#/orders">Back</Link></div>'+
...
async function viewActivity(){
  ...
  list.forEach(function(x){
    rows += '<tr><td class="py-2 px-2">'+x.id+'</td><td class="py-2 px-2">'+x.method+'</td><td class="py-2 px-2">'+x.url+'</td><td class="py-2 px-2">'+(x.idem||'')+'</td><td class="py-2 px-2">'+x.status+'</td><td class="py-2 px-2">'+x.ms+' ms</td><td class="py-2 px-2 text-right"><a class="btn btn-soft" href="#/activity/'+x.id+'">Open</Link></td></tr>';
  });
  document.getElementById('view').innerHTML =
...
async function viewActivityItem(id){
  ...
  document.getElementById('view').innerHTML =
    '<div class="mb-4 flex items-center justify-between"><h1 class="text-xl font-semibold">Activity #'+(x.id||id)+'</h1><a class="btn btn-soft" href="#/activity">Back</Link></div>'+
