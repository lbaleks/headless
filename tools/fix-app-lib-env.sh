#!/bin/bash
set -euo pipefail
echo "🔧 Moving env loader into app/api/_lib and fixing imports, then cleaning .next"

mkdir -p app/api/_lib

# 1) app/api/_lib/env.ts
cat > app/api/_lib/env.ts <<'TS'
// app/api/_lib/env.ts
...
# 2) Patch imports in product routes to use ../../_lib/env
for f in \
  app/api/products/\[sku]/route.ts \
  app/api/products/update-attributes/route.ts
do
  if [ -f "$f" ]; then
    echo "🛠  Updating import in $f -> ../../_lib/env"
    # replace any previous import to our new path
    sed -i.bak "s|from '../../../../lib/env'|from '../../_lib/env'|g" "$f" || true
    sed -i.bak "s|from '../../../lib/env'|from '../../_lib/env'|g" "$f" || true
    sed -i.bak "s|from '@/lib/env'|from '../../_lib/env'|g" "$f" || true
    sed -i.bak 's|from "@/lib/env"|from "../../_lib/env"|g' "$f" || true
    # ensure we import the right symbols
    sed -i.bak "s|{ getMagentoConfig, magentoUrl }|{ getMagentoConfig, magentoUrl }|g" "$f" || true
    rm -f "$f.bak"
  else
...
# 3) Clean .next to force full rebuild
if [ -d ".next" ]; then
  echo "🧹 Cleaning .next build output"
  rm -rf .next
fi
