...
log "Installerer Attribute Editor (UI + API-kobling)"
...
# injiser i produktside
grep -q AttributeEditor app/admin/products/[s]*/page.tsx 2>/dev/null || \
  sed -i '' '/<\/main>/i\
      import AttributeEditor from "@/src/components/AttributeEditor";\
      <AttributeEditor sku={sku} initial={item.attributes} />\
  ' app/admin/products/[s]*/page.tsx || true

log "Ferdig ✅  Åpne: /admin/products/TEST og rediger IBU/HOPS"
