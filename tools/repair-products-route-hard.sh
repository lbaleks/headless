#!/bin/bash
set -euo pipefail
echo "🔧 Re-skriver app/api/products/route.ts med ren, kompilérbar implementasjon"
...
# Clean build caches for a fresh dev build
rm -rf .next .next-dev node_modules/.cache 2>/dev/null || true
echo "🧹 Ryddet .next/.next-dev og caches"

echo "✅ Ferdig. Start på nytt: pnpm dev (hard refresh i nettleser)"
