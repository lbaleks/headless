...
BACKUP="$ROUTE.bak.$(date +%s)"
[ -f "$ROUTE" ] && cp "$ROUTE" "$BACKUP" || true
...
export async function GET(req: Request) {
  try {
    ...
    return serverError('Kunne ikke laste ordre', { detail: String(err?.message||err) })
  }
}
...
echo "✓ Skrev $ROUTE (backup: $BACKUP)"
echo "→ Start dev på nytt:  npm run dev"
echo "→ Test POST (uten jq først):"
