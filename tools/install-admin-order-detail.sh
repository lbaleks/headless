...
export function getOrder(id: string): Order | undefined {
  return mem.orders[id]
}

export function upsertOrder(o: Order) {
  mem.orders[o.id] = o
  return mem.orders[o.id]
}
...
export function addEvent(id: string, text: string, tone?: Order['events'][number]['tone']) {
  ...
  mem.orders[id] = { ...cur, events }
  return ev
}

// Helper for API shape
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Tillat delvis oppdatering
  const patch: {
    status?: OrderStatus
    notes?: string
    lines?: OrderLine[]
    eventText?: string
    eventTone?: 'info'|'success'|'warn'|'danger'|'neutral'
  } = body || {}
...
# ---------- app/admin/orders/[id]/OrderDetail.client.tsx ----------
...
export default function OrderDetail({ id }: { id: string }) {
  ...
  async function patch(patch: Partial<Pick<OrderDTO,'status'|'notes'|'lines'>> & {eventText?:string;eventTone?:Tone}) {
    setBusy(true)
    try {
      ...
      console.error(e)
      ;(window as any).lbToast?.('Kunne ikke lagre')
    } finally {
      setBusy(false)
    }
  }
  ...
  function total() {
    return (data?.lines || []).reduce((s,l)=>s + l.qty*l.price, 0)
  }

  function addLine() {
    if (!data) return
    const lines = [...data.lines, { name: 'Ny vare', qty: 1, price: 0 }]
    patch({ lines, eventText: 'La til varelinje', eventTone: 'info' })
  }

  function updateLine(ix:number, patchLine: Partial<OrderLine>) {
    if (!data) return
    const lines = data.lines.map((l,i)=> i===ix ? { ...l, ...patchLine } : l)
    patch({ lines })
  }

  function removeLine(ix:number) {
    if (!data) return
    const lines = data.lines.filter((_,i)=>i!==ix)
    patch({ lines, eventText: 'Fjernet varelinje', eventTone: 'warn' })
  }

  if (error) {
    return (
      <div className="p-4">
        <div className="text-red-700 text-sm">Feil: {error}</div>
        <button className="btn btn-sm mt-2" onClick={load}>Prøv igjen</button>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-neutral-500">
            Opprettet {new Date(data.createdAt).toLocaleString()}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <select
            ...
            disabled={busy}
          >
            {['draft','submitted','paid','fulfilled','cancelled'].map(s=>(
      ...
      <div className="border-b">
        {(['summary','lines','events'] as const).map(t=>(
          <button
            key={t}
            className={cn(
              'px-3 py-2 text-sm mr-1 rounded-t',
              tab===t ? 'bg-neutral-900 text-white' : 'hover:bg-neutral-100'
            )}
            onClick={()=>setTab(t)}
          >
      ...
      {tab==='summary' && (
        <div className="p-4 rounded border bg-white space-y-3">
          ...
          <div className="text-sm text-neutral-500">
            Ordre-ID: {data.id} • Opprettet: {new Date(data.createdAt).toLocaleString()}
          </div>
        </div>
      )}
      ...
      {tab==='lines' && (
        <div className="p-4 rounded border bg-white">
          ...
          <div className="overflow-x-auto">
                  ...
                  <tr key={ix}>
                    ...
                    <td className="p-2">
                      <input
                        className="w-full border rounded px-2 py-1"
                        value={l.sku || ''}
                        onChange={(e)=>updateLine(ix,{ sku: e.target.value })}
                      />
                    </td>
                    <td className="p-2 text-right">
                      <input
                        ...
                        value={l.qty}
                        onChange={(e)=>updateLine(ix,{ qty: Math.max(0, Number(e.target.value||0)) })}
                      />
                    </td>
                    <td className="p-2 text-right">
                      <input
                    ...
                    <td className="p-2 text-right">{currency(l.qty*l.price)}</td>
                    <td className="p-2 text-right">
                      <button className="text-red-600 hover:underline" onClick={()=>removeLine(ix)}>Fjern</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {data.lines.length===0 && (
              <div className="text-sm text-neutral-500 p-2">Ingen varelinjer</div>
            )}
          </div>
          <div className="mt-3 text-right text-sm">
            <span className="mr-2 text-neutral-500">Totalt:</span>
          ...
          <div className="space-y-3">
            {(data.events||[]).map(e=>(
              <div key={e.id} className="relative pl-4">
                <div className={cn('absolute left-0 top-1.5 h-2 w-2 rounded-full', {
                  ['bg-blue-600']: e.tone==='info',
                  ['bg-green-600']: e.tone==='success',
                  ['bg-amber-500']: e.tone==='warn',
                  ['bg-red-600']: e.tone==='danger',
                  ['bg-neutral-300']: !e.tone || e.tone==='neutral',
                })}/>
            ...
            {(!data.events || data.events.length===0) && (
              <div className="text-sm text-neutral-500">Ingen hendelser enda</div>
            )}
          </div>
        </div>
      )}
...
echo "✓ Ferdig. Start dev-server på nytt (npm run dev / yarn dev / pnpm dev) og åpne /admin/orders/<id>."