# tools/install-order-create-ui.sh
#!/usr/bin/env bash
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.."; pwd)"

echo "→ Sørger for mapper…"
mkdir -p "$ROOT/app/admin/orders/new"

TARGET="$ROOT/app/admin/orders/new/OrderCreate.client.tsx"
...
echo "→ Beholder eksisterende imports/route for POST i /api/orders (ingen endring nå)."

# env hint
if ! grep -q '^NEXT_PUBLIC_PRICE_MULTIPLIER=' "$ROOT/.env.local" 2>/dev/null; then
  echo "→ Legger til NEXT_PUBLIC_PRICE_MULTIPLIER i .env.local (1.25)"
  echo "NEXT_PUBLIC_PRICE_MULTIPLIER=1.25" >> "$ROOT/.env.local"
fi

echo "→ Rydder .next/.next-cache…"
rm -rf "$ROOT/.next" "$ROOT/.next-cache" 2>/dev/null || true

echo "✓ Ferdig. Start dev på nytt (npm run dev) og test /admin/orders/new."