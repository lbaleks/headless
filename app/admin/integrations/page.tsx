...
export default function IntegrationsPage(){
  ...
  return (
    <AdminPage title="Integrations">
          ...
          <tbody>
            {items.map(p=>(
              <tr key={p.key} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="p-3 border-t text-gray-600">{p.note||'—'}</td>
      ...
      {/* Modal */}
      {open && (
        <div className="fixed inset-0 bg-black/40 grid place-items-center z-50"  role="button" tabIndex={0} onKeyDown={(e)=>{if(e.key==="Enter"||e.key===" "){(e.currentTarget as any).click();}}} onClick={close}>
          <div className="bg-white rounded-2xl p-4 w-[min(720px,92vw)]"  role="button" tabIndex={0} onKeyDown={(e)=>{if(e.key==="Enter"||e.key===" "){(e.currentTarget as any).click();}}} onClick={e=>e.stopPropagation()}>
