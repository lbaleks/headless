...
export async function PUT(req:Request){
  try{
    ...
    const incoming = Array.isArray((body as any)?.rules) ? (body as any).rules : [];
    const clean: Rule[] = [];
    ...
    audit({ actor:"admin", action:"pricing.rules.update", target:"rules", meta:{ count: clean.length, via:"/api/rules" } });
    return NextResponse.json({ ok:true, rules: pricingDB.rules });
  } catch (e:any){
