  ...
  async function syncOne(i: number) {
    ...
    if (j?.ok) {
      setRows((cur) => cur.map((x, idx) => (idx === i ? j.item : x)));
        try {
          const prev: any = (typeof rows !== "undefined" ? rows?.[i] : undefined);
const sku   = (j?.item?.sku) ?? (prev?.sku ?? prev?.name);
const qty   = (typeof j?.item?.stock === "number" ? j.item.stock : (typeof prev?.stock === "number" ? prev.stock : undefined));
const price = (typeof j?.item?.price === "number" ? j.item.price : (typeof prev?.price === "number" ? prev.price : undefined));
