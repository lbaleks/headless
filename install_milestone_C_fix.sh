...
# 0) .env: RBAC keys (append hvis mangler)
...
# 5) Patching server.js med Node (ingen sed/perl-regex)
node <<'NODE'
const fs = require('fs');
...
s = ensureImport(s, 'rbac', './plugins/auth.rbac.js');
s = ensureImport(s, 'openapi', './plugins/openapi.js');
s = ensureImport(s, 'm2Invoices', './plugins/magento2.invoices.js');

if (!s.includes('register(rbac)')) {
  s = insertAfter(s, 'await app.register(cors', 'await app.register(rbac)');
}
if (!s.includes('register(openapi)')) {
  s = insertAfter(s, 'await app.register(m2SalesRules)', 'await app.register(openapi)');
}
if (!s.includes('register(m2Invoices)')) {
  s = insertAfter(s, 'await app.register(m2CreditMemos)', 'await app.register(m2Invoices)');
}
...
echo "🔑 Whoami (admin):"; curl -sS --max-time 5 -H 'x-api-key: dev-admin-key' "http://127.0.0.1:$PORT/v2/auth/whoami" | jq -c . || true
