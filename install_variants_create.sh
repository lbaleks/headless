...
PLUG="$API_DIR/plugins/variants.create.js"
...
# --- registrer plugin i server.js om ikke finnes ---
if ! grep -q "import variantsCreate from './plugins/variants.create.js'" "$SRV"; then
  gsed -i '' "1 a\\
import variantsCreate from './plugins/variants.create.js'\\
" "$SRV" 2>/dev/null || sed -i '' "1s;^;import variantsCreate from './plugins/variants.create.js'\\
;" "$SRV"
fi
if ! grep -q "await app.register(variantsCreate)" "$SRV"; then
  gsed -i '' "s|await app.register(rbac)|await app.register(rbac)\\
await app.register(variantsCreate)|" "$SRV" 2>/dev/null || \
sed -i '' "s|await app.register(rbac)|await app.register(rbac)\nawait app.register(variantsCreate)|" "$SRV"
fi
...
echo "⚙️  Endpoints:"
echo "POST /v2/integrations/magento/variants/bootstrap?from=TEST&cfg=TEST-CFG&attr=size"
echo "POST /v2/integrations/magento/products/TEST-CFG/variants  (body: { sku, value, label?, price?, weight? })"
