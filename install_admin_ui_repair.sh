...
# --- Skriv admin.ui.js på nytt (trygg versjon m/ søk + paginering) ---
cat > "$UI" <<'JS'
...
'async function viewProducts(){var params=getParams(); var q=params.get("q")||""; var page=Number(params.get("page")||1); var pageSize=12;'+
'  const r=await api("/v2/integrations/magento/products?page="+page+"&pageSize="+pageSize+"&q="+encodeURIComponent(q)); const j=await r.json(); const items=j.items||[];'+
'  const total=j.total||j.total_count||items.length; const pages=Math.max(1,Math.ceil(total/pageSize));'+
'  var cards=""; items.forEach(function(p){cards+= "<div class=\\"card\\"><div class=\\"font-semibold mb-1\\">"+(p.name||"")+"</div><div class=\\"text-sm text-slate-600 mb-2\\">"+(p.sku||"")+"</div><div class=\\"flex items-center justify-between\\"><span class=\\"text-sm\\">"+fmtMoney(p.price)+" · <span class=\\"tag\\">"+(p.status===1?"Enabled":"Disabled")+"</span></span><a class=\\"btn btn-soft\\" href=\\"#/product/"+encodeURIComponent(p.sku||"")+ "\\">Open</a></div></div>";});'+
'  document.getElementById("view").innerHTML = '+
'    "<div class=\\"mb-4 flex items-center justify-between\\"><h1 class=\\"text-xl font-semibold\\">Products</h1>"+'+
'    "<form id=\\"searchForm\\" class=\\"flex gap-2\\"><input id=\\"searchInput\\" class=\\"input\\" type=\\"text\\" placeholder=\\"Search…\\" value=\\"'+
'    q.replace(/"/g,"&quot;")+'+\\"\\"><button class=\\"btn btn-primary\\">Search</button></form></div>"+'+
...
'  var prev=document.getElementById("prevPage"); if(prev) prev.onclick=function(){ if(page>1) setHash("#/products",{q:q,page:page-1}) };'+
'  var next=document.getElementById("nextPage"); if(next) next.onclick=function(){ if(page<pages) setHash("#/products",{q:q,page:page+1}) };'+
'};'+

'async function viewProduct(sku){ const r=await api("/v2/integrations/magento/products/"+encodeURIComponent(sku)); const j=await r.json(); const p=j.item||{};'+
'  document.getElementById("view").innerHTML='+
...
'    "<div class=\\"card\\"><div class=\\"text-sm text-slate-600 mb-2\\">SKU: "+(p.sku||"")+"</div><div class=\\"mb-2\\">Price: <b>"+fmtMoney(p.price)+"</b></div><div>Status: <span class=\\"tag\\">"+(p.status===1?"Enabled":"Disabled")+"</span></div></div>";'+
'};'+

'async function viewOrders(){ var params=getParams(); var q=params.get("q")||""; var page=Number(params.get("page")||1); var pageSize=10;'+
'  const r=await api("/v2/integrations/magento/orders?page="+page+"&pageSize="+pageSize+"&q="+encodeURIComponent(q)); const j=await r.json(); const list=j.items||[];'+
'  const total=j.total||j.total_count||list.length; const pages=Math.max(1,Math.ceil(total/pageSize));'+
'  var rows=""; list.forEach(function(o){ rows+= "<tr class=\\"border-b\\"><td class=\\"py-2 px-2\\">"+o.entity_id+"</td><td class=\\"py-2 px-2\\">"+(o.increment_id||"")+"</td><td class=\\"py-2 px-2\\">"+(o.status||"")+"</td><td class=\\"py-2 px-2\\">"+fmtMoney(o.grand_total)+"</td><td class=\\"py-2 px-2 text-right\\"><a class=\\"btn btn-soft\\" href=\\"#/order/"+o.entity_id+"\\">Open</a></td></tr>"; });'+
'  document.getElementById("view").innerHTML='+
'    "<div class=\\"mb-4 flex items-center justify-between\\"><h1 class=\\"text-xl font-semibold\\">Orders</h1>"+'+
'    "<form id=\\"orderSearch\\" class=\\"flex gap-2\\"><input id=\\"orderInput\\" class=\\"input\\" type=\\"text\\" placeholder=\\"Search ID / keyword\\" value=\\"'+
'    q.replace(/"/g,"&quot;")+'+\\"\\"><button class=\\"btn btn-primary\\">Search</button></form></div>"+'+
'    "<div class=\\"card overflow-auto\\"><table class=\\"min-w-full text-sm\\"><thead><tr class=\\"text-left text-slate-500\\"><th class=\\"py-2 px-2\\">ID</th><th class=\\"py-2 px-2\\">Increment</th><th class=\\"py-2 px-2\\">Status</th><th class=\\"py-2 px-2\\">Total</th><th></th></tr></thead><tbody>"+rows+"</tbody></table></div>"+'+
...
echo
echo "🧭 Routes:"; curl -sS --max-time 6 "http://127.0.0.1:$PORT/v2/debug/routes" || true
echo
echo "🔎 Check admin route (expect HTML):"
curl -sI --max-time 6 "http://127.0.0.1:$PORT/v2/admin" || true
echo
echo "🪵 Tail log:"; tail -n 80 "$LOG" || true
